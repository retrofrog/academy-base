...
```bash
AIceBear@htb[/htb]$ sudo nmap 10.129.2.28 -p- -oA target
...
```bash
AIceBear@htb[/htb]$ cat target.gnmap
...
```bash
AIceBear@htb[/htb]$ cat target.xml
...
```bash
AIceBear@htb[/htb]$ xsltproc target.xml -o target.html
```
