...
```shell-session
AIceBear@htb[/htb]$ sudo impacket-smbserver share -smb2support /tmp/smbshare
...
```shell-session
AIceBear@htb[/htb]$ sudo impacket-smbserver share -smb2support /tmp/smbshare -user test -password test
...
```shell-session
AIceBear@htb[/htb]$ sudo pip3 install pyftpdlib
```
...
```shell-session
AIceBear@htb[/htb]$ sudo python3 -m pyftpdlib --port 21
...
```shell-session
AIceBear@htb[/htb]$ echo  | base64 -d > hosts
```

Windows File Transfer Methods

```shell-session
AIceBear@htb[/htb]$ md5sum hosts 
...
```shell-session
AIceBear@htb[/htb]$ pip3 install uploadserver
...
```shell-session
AIceBear@htb[/htb]$ echo <base64> | base64 -d -w 0 > hosts
```
...
```shell-session
AIceBear@htb[/htb]$ sudo pip install wsgidav cheroot
...
```shell-session
AIceBear@htb[/htb]$ sudo wsgidav --host=0.0.0.0 --port=80 --root=/tmp --auth=anonymous 
...
```shell-session
AIceBear@htb[/htb]$ sudo python3 -m pyftpdlib --port 21 --write
